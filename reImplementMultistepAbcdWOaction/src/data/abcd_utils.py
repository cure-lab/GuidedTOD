...
'''
context w/o actions; predict all the remaining actions
'''
def parse_abcd_dataset_for_ast_wo_action_multistep(raw_data_path: Path, data: List, data_type: str, sample_numbers = 80):
    non_enumerable, enumerable, mappers, value_by_action = prepare_labels_for_ast(raw_data_path)
    parsed_samples = []
    ...
    for sample in data:
        ...
        for idx, turn in enumerate(sample["delexed"]):
            if turn["speaker"] == "action":
                ...
                # get all the actions after this turn
                next_actions = []
                context__ = [c for c in context[:]]
                flag=True
                for idx_, turn_ in enumerate(sample["delexed"]):
                    if idx_ < idx:
                        continue
                    else:
                        if turn_["speaker"] == "action":
                            _, _, target_action_, target_values_, _ = turn_["targets"]
                            potential_values_ = value_by_action[target_action_]

                            if not target_values_ or target_values_ == [""]:
                                target_values_ = ["none"]
                            else:
                                corrected_target_values_ = []
                                for value in target_values_:
                                    context_str_ = " ".join(context__)
                                    corrected_value_ = ast_value_to_id(context_str_, value, potential_values_, enumerable)
                                    corrected_target_values_.append(corrected_value_)
                                target_values_ = corrected_target_values_
                            context__=[c for c in context__[:]]
                            context__.append(target_action_ + " " + str(target_values_))
                            
                            if flag:
                                target_action_ = sample['scenario']['flow'] + ":" + target_action_
                                flag = False
                            next_actions.append([target_action_, target_values_])
                        else:
                            context__.append(turn_["text"])
                ...
                parsed_samples.append({"context": context_, "action": next_actions, "convo_id": sample["convo_id"], "turn_id": turn["turn_count"], "flow": sample['scenario']['flow']})
            else:
                context.append(turn["text"])
    return parsed_samples
...
'''
add actions to context, and predict all the remaining actions
'''
def parse_abcd_dataset_for_ast_w_action_multistep(raw_data_path: Path, data: List, data_type: str, sample_numbers = 80):
    non_enumerable, enumerable, mappers, value_by_action = prepare_labels_for_ast(raw_data_path)
    parsed_samples = []
    ...
    for sample in data:
        ...
        for idx, turn in enumerate(sample["delexed"]):
            if turn["speaker"] == "action":
                ...
                parsed_samples.append({"context": context_, "action": next_actions, "convo_id": sample["convo_id"], "turn_id": turn["turn_count"]})
            else:
                context.append(turn["text"])
    return parsed_samples
