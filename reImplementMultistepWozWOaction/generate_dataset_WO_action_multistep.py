...
def convert_intents_to_ast(dialogue_intents_w_values, original_dialogue: List, action_mappings: Dict, turn_intent: Dict):
    ...
    for intent in intents:
        ...
        # for testing
        tmp_ast_sample = {}
        tmp_ast_sample["action"] = [action_name, action_values]
        # print("last_turn_id: ", last_turn_id)
        ...
        # for i in range(len(tmp_samples)):
        #     tmp_samples[i]['action'] = actions_all[i:]
        #     print("sample ", i, ": ", tmp_samples[i]['action'])
        #     parsed_samples.append(tmp_samples[i])

        # print("next_actions: ", tmp_ast_sample['action'])
    ...
    for sample in raw_data:
        ...
        for i in range(len(ASTs)):
            ...
            for j in range(i, len(ASTs)):
                all_next_actions.append(ASTs[j][1])
            # next_actions = ";".join([tmp_AST[1] for tmp_AST in ASTs])
            parsed_samples.append({"context": AST[2], "action": all_next_actions, "convo_id": sample["dialogue_id"], "turn_id": AST[0]})
        # print()
    return parsed_samples
            ...
            if speaker == 0:  # User
                for frame in active_frames["state"]:
                    ...
                    if len(action_values) == 0:
                        ...
                        action_values = ["none"]
                    # tmp_sample["action"] = [active_intent, action_values]
                    actions_all.append([active_intent, action_values])
                    # tmp_sample["context"] = [u[1] for u in original_dialogue[:int(turn_id)+1]]
                    tmp_sample["context"] = []
                    ...
                    action_history[int(turn_id)] = [active_intent, action_values]
                    
                    # if len(parsed_samples) != 0 and tmp_sample["action"] == parsed_samples[-1]["action"]:
                    #     continue
                    # parsed_samples.append(tmp_sample)
                    tmp_samples.append(tmp_sample)
        
        '''
        sample  0 :  [['find_train', ['none']], ['find_attraction', ['none']], ['find_attraction', ['swimmingpool']], ['find_attraction', ['swimmingpool']], ['find_train', ['friday', 'cambridge', 'leicester', '10 leaveat']], ['book_train', ['6 people']]]
        sample  1 :  [['find_attraction', ['none']], ['find_attraction', ['swimmingpool']], ['find_attraction', ['swimmingpool']], ['find_train', ['friday', 'cambridge', 'leicester', '10 leaveat']], ['book_train', ['6 people']]]
        sample  2 :  [['find_attraction', ['swimmingpool']], ['find_attraction', ['swimmingpool']], ['find_train', ['friday', 'cambridge', 'leicester', '10 leaveat']], ['book_train', ['6 people']]]
        sample  3 :  [['find_attraction', ['swimmingpool']], ['find_train', ['friday', 'cambridge', 'leicester', '10 leaveat']], ['book_train', ['6 people']]]
        sample  4 :  [['find_train', ['friday', 'cambridge', 'leicester', '10 leaveat']], ['book_train', ['6 people']]]
        sample  5 :  [['book_train', ['6 people']]]

        next_actions:  [['search-jacket', ['none']]]
        next_actions:  [['pull-up-account', ['rodriguez domingo']], ['verify-identity', ['rodriguez domingo', '<account_id>', '<account_id>']], ['validate-purchase', ['<order_id>', '<order_id>', '<order_id>']], ['validate-purchase', ['<username>', '<username>', '<username>']], ['ask-the-oracle', ['none']], ['update-order', ['change date']]]
        next_actions:  [['verify-identity', ['rodriguez domingo', '<account_id>', '<account_id>']], ['validate-purchase', ['<order_id>', '<order_id>', '<order_id>']], ['validate-purchase', ['<username>', '<username>', '<username>']], ['ask-the-oracle', ['none']], ['update-order', ['change date']]]
        next_actions:  [['validate-purchase', ['<order_id>', '<order_id>', '<order_id>']], ['validate-purchase', ['<username>', '<username>', '<username>']], ['ask-the-oracle', ['none']], ['update-order', ['change date']]]
        next_actions:  [['validate-purchase', ['<username>', '<username>', '<username>']], ['ask-the-oracle', ['none']], ['update-order', ['change date']]]
        next_actions:  [['ask-the-oracle', ['none']], ['update-order', ['change date']]]
        next_actions:  [['update-order', ['change date']]]
        '''
...
'''
Version for multi-step action: turn base (multiWoz)
'''
def create_ast_split_dataset_turnbase(parsed_data: List):
    ast_split_data = []
    for idx, sample in enumerate(parsed_data):
        ...
        for i in range(len(action)):
            actions_list.append(convert_workflow_to_str([action[i]]))

        # actions_list to string
...
def create_ast_split_dataset(parsed_data: List):
    ...
    for idx, sample in enumerate(parsed_data):
        ...
        # actions_list to string
        action_str = "; ".join(actions_list)
        # action_str = convert_workflow_to_str([action])
...
def create_ast_dataset(processed_data_path: Path, parsed_ast_data: Dict, dataset_name: str):
    for split, split_data in parsed_ast_data.items():
        # wd_split_data = create_ast_split_dataset_turnbase(split_data)
