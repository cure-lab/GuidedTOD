...
clientGPT3_5 = OpenAI(api_key="sk-89576ilSZoBXCFmGlJPxBEzpYGDRe17MBudXmnQmhMxmaC3x", base_url=base_url)
...
def eval_dialogue(predictions, labels, convo_ids, turn_ids, sequence_scores=None, num_beams=None, contexts=None):
    ...
    for new_pred, label1, new_sequence_score, convo_id1, turn_id1 in zip(new_predictions, labels, new_sequence_scores, convo_ids, turn_ids):
        ...
        for i in range(len(actions)):
            try:
                ...
                # TODO: Use the most similar action in the chained prior to replace the missing actions
    ...
    print("len(labels): ", len(labels))
    print("len(convo_ids): ", len(convo_ids))
    print("len(turn_ids): ", len(turn_ids))
    print("len(contexts): ", len(contexts))
    
    exp_name = "abcdASTWOActionFlowAll_with_chainedPrior2"
    ...
    return {
        ...
        "dialog_BLEU": round(dialog_BLEU, 4)
    }

    # return {
    #     "dialog_BLEU": round(dialog_BLEU, 4),
    #     "dialog_ROUGE": dialog_ROUGE,
    #     "dialog_BERT": dialog_BERT
    # }
...
def compute_ast_acc_metrics_DSP(predictions, labels, convo_ids, turn_ids, sequence_scores=None, num_beams=None):
    # print("len(predictions): ", len(predictions))
    # print("len(labels): ", len(labels))
    # print("len(sequence_scores): ", len(sequence_scores))
    from aalpy.utils import load_automaton_from_file
...
def compute_ast_acc_metrics_flow(predictions, labels, convo_ids, turn_ids, sequence_scores=None, num_beams=None, contexts=None):
    print("len(predictions): ", len(predictions))
    print("len(labels): ", len(labels))
    print("len(sequence_scores): ", len(sequence_scores))
    from aalpy.utils import load_automaton_from_file
...
def create_compute_metric_fct(tokenizer, data_args, training_args, model_args):
    ...
    def compute_ast_metrics(eval_preds, sequence_scores=None, num_beams=4):
        ...
        # return compute_ast_acc_metrics_flow(predictions, labels, conv_ids, turn_ids, sequence_scores, num_beams, contexts)
        ...
        return compute_ast_acc_metrics_DSP(predictions, labels, conv_ids, turn_ids, sequence_scores, num_beams)
