...
def parse_ast_prediction(prediction_str):
    ...
    # }
    # possibleActions = ['find_hotel', 'book_hotel', 'find_train', 'book_train', 'find_attraction', 'find_restaurant', 'book_restaurant', 'find_hospital', 'book_taxi', 'find_taxi', 'find_bus', 'find_police', 'MISSING']
...
'''
def compute_ast_acc_metrics_bak(predictions, labels, convo_ids, turn_ids, sequence_scores=None, num_beams=None):
    ...
    # load an automaton
    automaton = load_automaton_from_file("./chainPrior/learned_mdp_4000.dot", automaton_type='mdp')
    # print(automaton)
    ...
    for new_pred, label1, new_sequence_score, convo_id1, turn_id1 in zip(new_predictions, labels, new_sequence_scores, convo_ids, turn_ids):
        ...
        # merge_scores = 0.8*np.array(new_sequence_score) + 0.2*np.array(rates)
        # else:
...
def eval_dialogue(predictions, labels, convo_ids, turn_ids, sequence_scores=None, num_beams=None, contexts=None):
    print("len(predictions): ", len(predictions))
    print("len(labels): ", len(labels))
    print("len(sequence_scores): ", len(sequence_scores))
    from aalpy.utils import load_automaton_from_file
    ...
    for new_pred, label1, new_sequence_score, convo_id1, turn_id1 in zip(new_predictions, labels, new_sequence_scores, convo_ids, turn_ids):
        ...
        for i in range(len(actions)):
            try:
                if actions[i] not in action_mapping_reverse.keys():
                    rate = -10000
                else:
                    rate = transition_mapping[(action_mapping_reverse[previous_actions[-1]], action_mapping_reverse[actions[i]])][1]
            except:
                rate = -10000
            rates.append(rate)
        rates = np.array(rates)
    ...
    exp_name = "multiwozMultiASTWOActionFlowAll_with_chainedPrior2"

    if os.path.exists(f"dialogues/{exp_name}.json"):
        os.remove(f"dialogues/{exp_name}.json")

    counter = 0
    for pred, label, convo_id, turn_id, context in zip(new_new_predictions, labels, convo_ids, turn_ids, contexts):
        action_label, values_label = parse_ast_prediction(label)
        values_label.sort()
        # for value in values_label:
        #     action_labels.append(action_label)
    ...
    with open(f"dialogues/{exp_name}.json", "r") as r:
...
def compute_ast_acc_metrics(predictions, labels, convo_ids, turn_ids, sequence_scores=None, num_beams=None, contexts=None):
    print("len(predictions): ", len(predictions))
    print("len(labels): ", len(labels))
    print("len(sequence_scores): ", len(sequence_scores))
    from aalpy.utils import load_automaton_from_file
    ...
    all_flow_name = ['hotel', 'restaurant', 'attraction-restaurant', 'attraction-restaurant-taxi', 'hotel-restaurant', 'train', 'hotel-train', 'attraction', 'attraction-train', 'police', 'hotel-restaurant-taxi', 'attraction-hospital-restaurant', 'restaurant-train', 'restaurant-taxi', 'hospital', 'hospital-police', 'taxi', 'attraction-hotel', 'attraction-hotel-taxi', 'attraction-hotel-restaurant-taxi', 'attraction-restaurant-taxi-train', 'attraction-hotel-train', 'hospital-taxi', 'attraction-restaurant-train', 'attraction-hotel-taxi-train', 'attraction-hotel-restaurant', 'hospital-hotel-restaurant', 'hospital-restaurant', 'hospital-hotel', 'bus-train', 'bus-hotel-train', 'restaurant-taxi-train', 'hotel-taxi', 'attraction-hotel-restaurant-train', 'hotel-restaurant-train', 'hospital-restaurant-train', 'attraction-taxi-train', 'attraction-taxi', 'attraction-hospital-hotel', 'hospital-hotel-restaurant-taxi', 'taxi-train', 'hospital-train', 'hospital-hotel-train', '', 'police-restaurant', 'attraction-hospital-restaurant-taxi', 'hotel-taxi-train', 'police-train', 'hotel-police-train', 'attraction-hospital-hotel-taxi', 'police-taxi', 'attraction-hospital', 'hospital-hotel-restaurant-taxi-train', 'attraction-bus-restaurant-train', 'bus-taxi-train', 'attraction-bus-taxi-train', 'attraction-police', 'hotel-restaurant-taxi-train', 'attraction-police-train', 'police-restaurant-train', 'hospital-police-restaurant-train', 'attraction-police-restaurant-train', 'hotel-police', 'attraction-hospital-hotel-restaurant', 'attraction-hospital-hotel-restaurant-taxi']
    ...
    for new_pred, label1, new_sequence_score, convo_id1, turn_id1 in zip(new_predictions, labels, new_sequence_scores, convo_ids, turn_ids):
        ...
        current_flows = []
        for pred in new_pred:
            # get from the start to the first ':'
            # pred = label1
            tmp_pred = pred.split(' [')
            tmp_pred = tmp_pred[0].split(':')
            current_flows.append(tmp_pred[0].strip())
            ...
            if len(tmp_pred) == 2:
                ...
                action = tmp_pred[0]
            # print(label1)
            # action = label1.split(' [')[0].strip().split(':')[1].strip()
        ...
        for i in range(len(actions)):
            try:
                ...
                # # print((action_mapping_reverse[previous_actions[-1]], action_mapping_reverse[actions[i]]))
                # print()
                rate = -10000
            rates.append(rate)
        rates = np.array(rates)
        ...
        # print()
        # print(f"log_norm_exp_rates: {log_norm_exp_rates}")
        # merge_scores = 0.9*np.array(log_norm_exp_new_sequence_score) + 0.1*np.array(log_norm_exp_rates)
        ...
        # merge_scores = []
        # all_possible_actions = ['find_hotel', 'book_hotel', 'find_train', 'book_train', 'find_attraction', //'find_restaurant', 'book_restaurant', 'find_hospital', 'book_taxi', 'find_taxi', 'find_bus', 'find_police']
        ...
        # all_possible_actions = ['search for hotel', 'book hotel', 'search for trains', 'book train ticket', 'search for attractions', 'search for restaurants', 'book table at restaurant', 'search for hospital', 'book taxi', 'search for taxi', 'search for bus', 'search for police station']
        ...
        label1 = label1.split(' [')[0].strip()
        previous_actions.append(label1.split(':')[1].strip())
...
def compute_ast_acc_metrics_flow_dialogueLevel(predictions, labels, convo_ids, turn_ids, sequence_scores=None, num_beams=None, contexts=None):
    print("len(predictions): ", len(predictions))
    print("len(labels): ", len(labels))
    print("len(sequence_scores): ", len(sequence_scores))
    from aalpy.utils import load_automaton_from_file
    ...
    for new_pred, label1, new_sequence_score, convo_id1, turn_id1 in zip(new_predictions, labels, new_sequence_scores, convo_ids, turn_ids):
        ...
        label1 = label1.split(' [')[0].strip()
        # previous_actions.append(label1.split(':')[1].strip())
        previous_actions.append(label1)
    ...
    # print("len(new_new_predictions): ", len(new_new_predictions))
    # print("len(labels): ", len(labels))
    for pred, label, convo_id, turn_id, context in tqdm(zip(new_new_predictions, labels, convo_ids, turn_ids, contexts), total=len(new_new_predictions)):
        # tmp_pred_flow_action = pred.split(' [')[0].strip()
        # tmp_label_flow_action = label.split(' [')[0].strip()
        # tmp_pred_flow = tmp_pred_flow_action.split(':')[0].strip()
        # tmp_label_flow = tmp_label_flow_action.split(':')[0].strip()
        # tmp_pred = pred.replace(tmp_pred_flow+":", '')
        # tmp_label = label.replace(tmp_label_flow+":", '')
        tmp_label = label
        tmp_pred = pred
        ...
        action_preds.append(action_pred)
        value_preds.append(values_pred)
        # print(f"action_pred: {action_pred}, action_label: {action_label}")
...
def create_compute_metric_fct(tokenizer, data_args, training_args, model_args):
    ...
    def compute_ast_metrics(eval_preds, sequence_scores=None, num_beams=4):
        predictions, labels = parse_predictions(eval_preds)
        # is_eval = True if len(labels) == 6025 else False
        ...
        sequence_scores: (200,)
        '''
        # return compute_ast_acc_metrics(predictions, labels, conv_ids, turn_ids, sequence_scores, num_beams, contexts)
        return compute_ast_acc_metrics_flow_dialogueLevel(predictions, labels, conv_ids, turn_ids, sequence_scores, num_beams, contexts)
        # return eval_dialogue(predictions, labels, conv_ids, turn_ids, sequence_scores, num_beams, contexts)
